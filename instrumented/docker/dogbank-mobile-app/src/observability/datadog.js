...
const service = 'dogbank-mobile-app';
const env = 'dogbank';
const version = '1.0.0';
...
if (!globalScope.__DOGBANK_MOBILE_DATADOG_INITIALIZED__) {
  datadogLogs.init({
    clientToken,
    site,
    service,
  ...
  datadogLogs.logger.info('dogbank.mobile.logs.initialized', {
    route: globalScope.location?.pathname,
    source: 'react-mobile',
  });

  globalScope.__DOGBANK_MOBILE_DATADOG_INITIALIZED__ = true;
}
