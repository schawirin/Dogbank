...
const PixConfirmPage = () => {
  ...
  const handleError = (err) => {
    ...
    trackProductAction('dogbank.pix.transfer.failed', {
      amount,
      source_account_id: errorDetails.accountId,
      pix_key_type: state.pixKey?.includes('@') ? 'email' : 'other',
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    setCounter(3);
    trackProductAction('dogbank.pix.transfer.started', {
      amount,
      source_account_id: state.sourceAccountId ?? user?.accountId ?? localStorage.getItem('accountId'),
      pix_key_type: state.pixKey?.includes('@') ? 'email' : 'other',
    });
    ...
    try {
      ...
      trackProductAction('dogbank.pix.transfer.completed', {
        amount,
        source_account_id: state.sourceAccountId ?? user?.accountId ?? localStorage.getItem('accountId'),
        transaction_id: receipt?.id || receipt?.transactionId,
      });
  ...
  const handleRetry = () => {
    ...
    setPassword('');
    trackProductAction('dogbank.pix.transfer.retry_clicked', {
      amount,
      source_account_id: state.sourceAccountId ?? user?.accountId ?? localStorage.getItem('accountId'),
    });
  };
  ...
  const handleCloseModal = () => {
    setShowErrorModal(false);
    trackProductAction('dogbank.pix.transfer.error_closed', {
      amount,
