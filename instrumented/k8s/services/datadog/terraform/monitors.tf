...
resource "datadog_monitor" "transaction_service_error_rate" {
  name    = "[DogBank] Transaction Service - Error Rate > 1%"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "sum(last_5m):trace.servlet.request.errors{service:transaction-service,env:${var.environment}}.as_count() / sum:trace.servlet.request.hits{service:transaction-service,env:${var.environment}}.as_count() * 100 > 1"
  ...
  notify_no_data    = false
  renotify_interval = 30
  priority          = 2  # P2 = High (Service Map shows orange/red)
...
resource "datadog_monitor" "transaction_service_latency" {
  name    = "[DogBank] Transaction Service - Latency P99 > 500ms"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "percentile(last_5m):p99:trace.servlet.request{service:transaction-service,env:${var.environment}} > 0.5"
  ...
  notify_no_data    = false
  renotify_interval = 60
  priority          = 3  # P3 = Medium (Service Map shows yellow)
...
resource "datadog_monitor" "bancocentral_service_error_rate" {
  name    = "[DogBank] Banco Central Service - Error Rate > 1%"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "sum(last_5m):trace.servlet.request.errors{service:bancocentral-service,env:${var.environment}}.as_count() / sum:trace.servlet.request.hits{service:bancocentral-service,env:${var.environment}}.as_count() * 100 > 1"
...
resource "datadog_monitor" "bancocentral_service_latency" {
  name    = "[DogBank] Banco Central Service - Latency P99 > 500ms"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "percentile(last_5m):p99:trace.servlet.request{service:bancocentral-service,env:${var.environment}} > 0.5"
...
resource "datadog_monitor" "auth_service_error_rate" {
  name    = "[DogBank] Auth Service - Error Rate > 1%"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "sum(last_5m):trace.servlet.request.errors{service:auth-service,env:${var.environment}}.as_count() / sum:trace.servlet.request.hits{service:auth-service,env:${var.environment}}.as_count() * 100 > 1"
...
resource "datadog_monitor" "auth_service_latency" {
  name    = "[DogBank] Auth Service - Latency P99 > 500ms"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "percentile(last_5m):p99:trace.servlet.request{service:auth-service,env:${var.environment}} > 0.5"
...
resource "datadog_monitor" "account_service_error_rate" {
  name    = "[DogBank] Account Service - Error Rate > 1%"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "sum(last_5m):trace.servlet.request.errors{service:account-service,env:${var.environment}}.as_count() / sum:trace.servlet.request.hits{service:account-service,env:${var.environment}}.as_count() * 100 > 1"
...
resource "datadog_monitor" "account_service_latency" {
  name    = "[DogBank] Account Service - Latency P99 > 500ms"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "percentile(last_5m):p99:trace.servlet.request{service:account-service,env:${var.environment}} > 0.5"
...
resource "datadog_monitor" "pix_worker_error_rate" {
  name    = "[DogBank] PIX Worker - Error Rate > 1%"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "sum(last_5m):trace.servlet.request.errors{service:pix-worker,env:${var.environment}}.as_count() / sum:trace.servlet.request.hits{service:pix-worker,env:${var.environment}}.as_count() * 100 > 1"
...
resource "datadog_monitor" "pix_worker_latency" {
  name    = "[DogBank] PIX Worker - Latency P99 > 500ms"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "percentile(last_5m):p99:trace.servlet.request{service:pix-worker,env:${var.environment}} > 0.5"
...
resource "datadog_monitor" "chatbot_service_error_rate" {
  name    = "[DogBank] Chatbot Service - Error Rate > 1%"
  type    = "query alert"
  message = <<-EOT
...
EOT
  ...
  query = "sum(last_5m):trace.flask.request.errors{service:chatbot-service,env:${var.environment}}.as_count() / sum:trace.flask.request.hits{service:chatbot-service,env:${var.environment}}.as_count() * 100 > 1"
...
resource "datadog_monitor" "chatbot_service_latency" {
  name    = "[DogBank] Chatbot Service - Latency P99 > 2s"
  type    = "query alert"
  message = <<-EOT
## Chatbot Service com latencia alta!

**Latencia P99**: {{value}}s
**Threshold**: Critical > 2s, Warning > 1s
...
EOT
  ...
  query = "percentile(last_5m):p99:trace.flask.request{service:chatbot-service,env:${var.environment}} > 2"
...
# =============================================================================
# Chatbot Service - Rate Limit (429)
# =============================================================================
resource "datadog_monitor" "chatbot_rate_limit" {
  name    = "[DogBank] Chatbot Service - Rate Limit (429) Groq API"
  type    = "log alert"
  message = <<-EOT
## Chatbot atingindo rate limit da Groq API!

**Status**: 429 Too Many Requests
**Occorrências**: {{value}} em 5 minutos

### Impacto
Chatbot pode estar retornando erros para usuários devido a limite de requisições da API.

### Causas Possíveis
1. Volume de requisições muito alto
2. Plano gratuito da Groq atingido
3. Necessidade de upgrade do plano

### Investigação
1. Verificar volume de requests no LLM Observability
2. Checar limites do plano Groq
3. Verificar se há retry loops
4. Analisar padrão de uso (picos ou constante)

### Remediação
- **Curto prazo**: Implementar retry with exponential backoff
- **Médio prazo**: Adicionar caching de respostas comuns
- **Longo prazo**: Avaliar upgrade do plano ou multi-provider

### Links Úteis
- Groq Console: https://console.groq.com/settings/limits
- LLM Observability: Datadog > APM > LLM Observability

${var.slack_channel}
EOT

  query = "logs(\"service:chatbot-service status:error (\\\"429\\\" OR \\\"rate limit\\\" OR \\\"too many requests\\\")\").index(\"*\").rollup(\"count\").last(\"5m\") > 10"
